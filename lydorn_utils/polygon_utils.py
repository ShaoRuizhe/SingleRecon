...
def point_project_onto_geometry(coord, target):
    ...
    _, projected_point = shapely.ops.nearest_points(point, target.exterior)
...
def compute_contour_measure(pred_polygon, gt_contours, sampling_spacing, max_stretch, metric_name="cosine"):
    ...
    # return np.arccos(scalar_products)
...
def compute_polygon_contour_measures(pred_polygons: list, gt_polygons: list, sampling_spacing: float,
                                     min_precision: float, max_stretch: float, metric_name: str = "cosine",
                                     progressbar=False):
    """
    ...
    filtered_pred_polygons = [(pred_polygon, gt_polygon) for pred_polygon, gt_polygon in zip(pred_polygons, gt_polygons)
                              if gt_polygon.is_valid and min_precision < pred_polygon.intersection(
            gt_polygon).area / pred_polygon.area]
    ...
    half_tangent_max_angles = [
        compute_contour_measure(pred_polygon, gt_polygon, sampling_spacing=sampling_spacing, max_stretch=max_stretch,
                                metric_name=metric_name)
        for pred_polygon, gt_polygon in iterator]
    # return half_tangent_max_angles
    return np.nanmean(np.concatenate(half_tangent_max_angles))
...
def main():
    ...
    max_angle_diffs = compute_polygon_contour_measures(pred_polygons, gt_polygons, sampling_spacing=0.1,
                                                       min_precision=0.5, max_stretch=2)
