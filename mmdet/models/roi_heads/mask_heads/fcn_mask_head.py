...
class FCNMaskHead(nn.Module):
    ...
    def get_seg_masks(self, mask_pred, det_bboxes, det_labels, rcnn_test_cfg,
        ...
        im_mask = torch.zeros(
            ...
            dtype=torch.bool if threshold >= 0 else torch.float32).squeeze(1)

        if self.class_agnostic:
            mask_pred = mask_pred[range(N), 0][:, None]
        ...
        for i in range(N):
            cls_segms[labels[i]].append(im_mask[i].cpu().detach().numpy())
        return cls_segms
...
def _do_paste_mask(masks, boxes, img_h, img_w, skip_empty=True):
    ...
    if skip_empty:
        return img_masks.squeeze(1), (slice(y0_int, y1_int), slice(x0_int, x1_int))
    else:
        return img_masks.squeeze(1), ()
