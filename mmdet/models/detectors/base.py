...
class BaseDetector(nn.Module, metaclass=ABCMeta):
    ...
    def forward(self, img, img_metas, return_loss=True, **kwargs):
        ...
        # plt.imshow(
        #     tensor2imgs(img[0], img_metas[0][0]['img_norm_cfg']['mean'], img_metas[0][0]['img_norm_cfg']['std'])[0])
        # plt.show()
        if return_loss:
    ...
    def train_step(self, data, optimizer):
        ...
        # from mmdet.core import tensor2imgs
        # self.show_result(
        #     tensor2imgs(data['img'][:1], **(data['img_metas'][0]['img_norm_cfg']))[0],
        #     {'bbox': data['gt_bboxes'][0].cpu(),'segm':data['gt_masks'][0].masks},num_obj=60)
    ...
    def val_step(self, data, optimizer):
        ...
        """
        losses = self(**data,return_loss=False)# bonai中未使用这个val，使用的是自己写的val hook。其中使用的是single_gpu_test,调用model.forward
        loss, log_vars = self._parse_losses(losses)
