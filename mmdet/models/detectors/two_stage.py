...
class TwoStageDetector(BaseDetector):
    ...
    def __init__(self,
        ...
        if pretrained is None and backbone.get('init_cfg') is not None:
            if backbone.init_cfg.get('type')=='Pretrained':
                pretrained=backbone.init_cfg.checkpoint
    ...
    def simple_test(self, img, img_metas, proposals=None, rescale=False):
        ...
        # for i in range(100):
        #     bbox = proposal_list[0][i, :4].cpu().numpy()
        #     ax = plt.gca()
        #     ax.add_patch(
        ...
        for i in range(x[0].shape[0]):# roi head一个一个图像处理，这是由于使用了RoIAlign，只能使用单个图像
            ...
            result.append(self.roi_head.simple_test(
