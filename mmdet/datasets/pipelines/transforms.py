...
class RandomFlip(object):
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            if 'gt_crossfields' in results:
                results['gt_crossfields'] =np.pi- results['gt_crossfields']#  除了上面的翻转，crossfield还需要将角度也进行翻转 无论哪个方向都是180-corssfield
...
class Normalize(object):
    ...
    def __call__(self, results):
        ...
        """
        for key in results.get('img_fields', ['img']):
            if key in ['gt_segs','gt_crossfields','instance_indicator']:
                continue
...
@PIPELINES.register_module
class RotateAccordingToAnno(RandomRotate):
    '''根据Anno中的某个角度进行旋转'''
    def __init__(self,angle_anno,auto_bound=False):
        self.angle_anno=angle_anno
        self.auto_bound=auto_bound

    def __call__(self, results):# todo:auto_bound需要改很多
        results['rotate']=True
        results['rotate_angle']=-results[self.angle_anno]*180/np.pi
        # offset原始计算角度为顺时针为正，mmcv.imrotate的旋转也是顺时针(本质上是cv取负，mmcv再取负，详见mmcv.imrotate代码），
        # 因此要通过旋转将offset归零，就要mmcv.imrotate(-gt_angle)
        if 'img' in results:
            results['img'] = mmcv.imrotate(
                results['img'], results['rotate_angle'], auto_bound=self.auto_bound)
        else:
            results['img_shape']=(results['img_info']['height'],results['img_info']['width'])
        # rotate offsets
        for key in results.get('offset_fields', []):
            results[key] = self.offset_rotate(results[key],
                                              results['img_shape'],
        ...
        if 'gt_bboxes' in results:
            if 'roof_masks' in results['ann_info'] and 'gt_offsets' in results:
                roof_polys=results['ann_info']['roof_masks']
                offsets=results['gt_offsets']
                rotated_bboxes=[]
                M = cv2.getRotationMatrix2D([results['img_shape'][0]/2,results['img_shape'][1]/2],
                                            -results['rotate_angle'], 1)
                ...
                if self.auto_bound: # 需要增加一个平移，平移距离等于图像因旋转而增加的尺寸的一半
                    M[0,2]+=(results['img'].shape[0]-results['img_shape'][0])/2
                    M[1,2]+=(results['img'].shape[1]-results['img_shape'][1])/2
                rm_objs=[]
                for i,(roof_poly,offset) in enumerate(zip(roof_polys,offsets)):
                    poly=np.array(roof_poly).reshape((-1,2))
                    for j in range(len(poly)):
                        poly[j]=M.dot([poly[j,0],poly[j,1],1])
                    left=min(poly[:,0])
                    top=min(poly[:,1])
                    right=max(poly[:,0])
                    bottom=max(poly[:,1])
                    ...
                    if offset[1]>0:
                        ...
                        bottom-=offset[1]
                    if left<1024 and top<1024 and right>0 and bottom>0:# 转出去的就不要了
                        rotated_bboxes.append([left,top,right,bottom])
                    else:
                        rm_objs.append(i)
                results['gt_labels']=np.delete(results['gt_labels'],rm_objs,0)
                results['gt_masks'].masks=np.delete(results['gt_masks'].masks,rm_objs,0)
                results['gt_offsets'] = np.delete(results['gt_offsets'], rm_objs,0)
                results['gt_bboxes']=np.array(rotated_bboxes).astype(np.float32)
            else:
                results['gt_bboxes'] = self.bbox_rotate(results['gt_bboxes'],
                                                results['img_shape'],
                                                results['rotate_angle'])
        # rotate masks
        for key in results.get('mask_fields', []):
            masks = [
                mmcv.imrotate(mask, results['rotate_angle'], auto_bound=self.auto_bound)
                for mask in results[key]
            ]
            if masks:
                rotated_masks = np.stack(masks)
...
@PIPELINES.register_module
class RandomCenterCropPadRotated(RandomCenterCropPad):
    pass