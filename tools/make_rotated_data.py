...
def rotate_with_anno(anno,img,angle,auto_bound=False):
    ...
    for anno_obj in anno:
        ...
        if left < 1024 and top < 1024 and right > 0 and bottom > 0:  # 转出去的就不要了 todo：开启auto bound的情况图像尺寸不为1024？
            rotated_anno_obj['building_bbox']=([left, top, right-left, bottom-top]) # 读取数据过程中的BONAI._frase_ann_info中会将标注中的box以x,y,w,h的形式读入，并通过x,y,x+w,y+h转化为x1,y1,x2,y2形式
...
def make_rotated_data(split='trainval'):# trainval/test
    cfg = Config.fromfile('configs/loft_foa/loft_foa_r50_fpn_2x_bonai.py')
    out_dir=r'D:\Documents\Dataset\building_footprint\BONAI\rotated\\'+split
    if split=='trainval':
        img_path=cfg.data.train.img_prefix[0]
...
def cat_train_json():
    # 由于train数据集的标注json是分城市的，所以需要将这些汇合到一起
    cfg = Config.fromfile('configs/loft_foa/loft_foa_r50_fpn_2x_bonai.py')
    out_dir = r'D:\Documents\Dataset\building_footprint\BONAI\coco\bonai_cat_train.json'

    anno_path = cfg.data.train.ann_file
    json_coco = {'annotations': [], 'images': [], 'categories': [{'id': 1, 'name': 'building'}]}
    past_json_images=0
    anno_id=1
    for json_file in anno_path:
        ...
        for anno in dataset['annotations']:
            anno['id']=anno_id
            anno['image_id']=anno['image_id']+past_json_images
            json_coco['annotations'].append(anno)
            anno_id+=1
        for img in dataset['images']:
            img['id']=img['id']+past_json_images # 注意要保证anno和img id的唯一性，这是coco读取json数据的要求
            json_coco['images'].append(img)
        past_json_images=img['id']
    json_coco['categories']=dataset['categories']
    json_coco['info']=dataset['info']
    json_coco['licenses']=dataset['licenses']
    json_coco['type']=dataset['type']
...
if __name__ == '__main__':
    ...
    # print('test is done, processing train')
    # make_rotated_data('trainval')