...
class FrameFieldModelOffNadir(FrameFieldModel):
    def __init__(self, config: dict, backbone, train_transform=None, eval_transform=None):
        """
        ...
        :param eval_transform: transform applied to the inputs when self.training is False
        """
        super(FrameFieldModelOffNadir, self).__init__(config,backbone,train_transform,eval_transform)

        if self.config["compute_seg"]:
            self.interior_seg_channels=self.config["seg_params"]["compute_interior"]
            self.edge_seg_channels=self.config["seg_params"]["compute_edge"]
            self.vertex_seg_channels=self.config["seg_params"]["compute_vertex"]
    ...
    def inference(self, image):
        ...
        if self.config["compute_seg"]:
            ...
            seg_to_cat = seg.clone().detach()
            backbone_features = torch.cat([backbone_features, seg_to_cat], dim=1)  # Add seg to image features
            outputs["interior_seg"] = seg[:,:self.interior_seg_channels]
            outputs["edge_seg"] = seg[:,self.interior_seg_channels:self.interior_seg_channels+self.edge_seg_channels]
            outputs["vertex_seg"] = seg[:,self.interior_seg_channels+self.edge_seg_channels:self.interior_seg_channels+self.edge_seg_channels+self.vertex_seg_channels]
