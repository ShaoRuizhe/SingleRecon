...
def polygonize(seg_batch, config, pool=None, pre_computed=None):
    ...
    if isinstance(indicator_batch,torch.Tensor):
        indicator_batch = indicator_batch.cpu().numpy()
    # toc = time.time()
    # debug_print(f"Indicator to cpu: {toc - tic}s")

    if pre_computed is None or "init_contours_batch" not in pre_computed:
        # tic = time.time()
        init_contours_batch = polygonize_utils.compute_init_contours_batch(indicator_batch, config["data_level"], pool=pool)
        # toc = time.time()
    ...
    if pool is not None:
        ...
        polygons_probs_batch = pool.starmap(shapely_postprocess_partial, zip(init_contours_batch, indicator_batch))
        polygons_batch, probs_batch = zip(*polygons_probs_batch)
    else:
        polygons_batch = []
        probs_batch = []
        for i, out_contours in enumerate(init_contours_batch):
            polygons, probs = shapely_postprocess(out_contours, indicator_batch[i], config)
