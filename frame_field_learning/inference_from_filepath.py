...
from .model import FrameFieldModel, FrameFieldModelOffNadir
...
def inference_from_filepath(config, in_filepaths, backbone, out_dirpath=None):
    ...
    if config['loss_params'].get('losses_set_name')=='off_nadir':
        model = FrameFieldModelOffNadir(config, backbone=backbone, eval_transform=eval_online_cuda_transform)
    else:
        model = FrameFieldModel(config, backbone=backbone, eval_transform=eval_online_cuda_transform)
    model.to(config["device"])
    if config.get('checkpoint_filepath') is not None:
        model.load_state_dict(torch.load(config['checkpoint_filepath'])['model_state_dict'])
    else:
    ...
    for in_filepath in pbar:
        ...
        sample = {
            "image": torchvision.transforms.functional.to_tensor(image)[None, ...],# 注意这里是自己写的torch_lydorn里面的to_tensor,并不会将图像转换到0-1的float。。。
