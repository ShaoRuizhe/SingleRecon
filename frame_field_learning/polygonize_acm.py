...
class PolygonAlignLoss:
    ...
    def __call__(self, tensorpoly):
        ...
        :return:
        """
        polygon = tensorpoly.pos[tensorpoly.to_padded_index]# 顶点坐标？估计是
        polygon_batch = tensorpoly.batch[tensorpoly.to_padded_index]# polygon处于哪个batch

        # Compute edges:
        edges = polygon[1:] - polygon[:-1]# 边向量
        ...
        edge_mask[tensorpoly.to_unpadded_poly_slice[:-1, 1]] = 0# to_unpadded_poly_slice是每个多变新的顶点在tensorpoly中的index范围（起始点和终止点）。这里是将不同多边形之间的边mask改成0

        midpoints = (polygon[1:] + polygon[:-1]) / 2# 边的中点坐标
        ...
        midpoints_c0 = self.c0c2[midpoints_batch, :2, midpoints_int[:, 0], midpoints_int[:, 1]]# 中点位置的frame方向（c0c2表示）
        midpoints_c2 = self.c0c2[midpoints_batch, 2:, midpoints_int[:, 0], midpoints_int[:, 1]]

        norms = torch.norm(edges, dim=-1)# norm：求范数，即边长
        ...
        z = edges / (norms[:, None] + 1e-3)# 归一化

        # Align to crossfield
        # align_loss = frame_field_utils.framefield_align_error(midpoints_c0, midpoints_c2, z, complex_dim=1)# 计算4次多项式作为loss
        align_loss = frame_field_utils.framefield_align_error_2level(midpoints_c0, midpoints_c2, z, complex_dim=1)# 计算4次多项式作为loss
        align_loss = align_loss * edge_mask
        total_align_loss = torch.sum(align_loss)

        # Align to level set of indicator:
        pos_indicator_value = bilinear_interpolate(self.indicator[:, None, ...], tensorpoly.pos, batch=tensorpoly.batch)# self.indicator就是建筑物分割mask，这是通过双线性插值将定点位置的mask取出来
        ...
        level_loss = torch.sum(torch.pow(pos_indicator_value - self.level, 2))# 定点处的建筑物分割mask应当为self.level=0.5
        ...
        losses_dict = {
            "align": total_align_loss.item(),# 边的方向与frame一致
            "level": level_loss.item(),# 定点位置的建筑物分割mask接近0.5
            "length": length_penalty.item(),# 建筑物的边长尽可能短“ensures that the node distribution along paths remains homogeneous as well as tight.”
        }
...
class TensorPolyOptimizer:
    ...
    def step(self, iter_num):
        ...
        self.optimizer.step()# self.tensorpoly.pos是有梯度的
        self.lr_scheduler.step(iter_num)

        # Move endpoints back:
        with torch.no_grad():
            self.tensorpoly.pos[self.tensorpoly.is_endpoint] = self.endpoint_pos# 每个polygon的起点和终点位置tensorpoly.is_endpoint为T，其他为F。此处调整了终点处的顶点位置
...
def polygonize(seg_batch, crossfield_batch, config, pool=None, pre_computed=None):
    ...
    if len(init_contours_batch[0])==0:
        return [],[]
    ...
    for i, out_contours in enumerate(out_contours_batch[0]):
        plt.plot(out_contours[:,0],out_contours[:,1])
    plt.show()
    for i, out_contours in enumerate(init_contours_batch[0]):
        plt.plot(out_contours[:,0],out_contours[:,1])
    plt.show()
