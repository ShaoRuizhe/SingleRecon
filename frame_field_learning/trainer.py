...
from .frame_field_utils import show_crossfield
...
class Trainer:
    def __init__(self, rank, gpu, config, model, optimizer, loss_func,
        ...
        self.gpu = gpu
        self.config = config
        if config['dataset_params'].get('off_nadir',False):
            self.focus_iou='interior[1]'# roof
        else:
            self.focus_iou=''
        self.model = model
    ...
    def loss_batch(self, batch, opt=None, epoch=None):
        ...
        vis_batch={'crossfield':pred['crossfield'],'image':batch['image']}
        show_crossfield(vis_batch)
        ...
        for i,seg_name in enumerate(['interior','edge','vertex']):
            if seg_name+'_seg' in pred and pred[seg_name+'_seg'].shape[1]>0:
                for class_num in range(1,pred[seg_name+'_seg'].shape[1]):
                    y_pred = pred[seg_name+'_seg'][:, class_num, ...]
                    y_true = batch["gt_polygons_image"][:, i, ...]==class_num
                    iou_thresholds = [0.1, 0.25, 0.5, 0.75, 0.9]
                    for iou_threshold in iou_thresholds:
                        iou = measures.iou(y_pred.reshape(y_pred.shape[0], -1), y_true.reshape(y_true.shape[0], -1),
                                           threshold=iou_threshold)
                        mean_iou = torch.mean(iou)
                        individual_metrics_dict[f"{seg_name}[{class_num}]IoU_{iou_threshold}"] = mean_iou
        ...
        # Log IoU if exists
        log_iou = None
        iou_name = self.focus_iou+f"IoU_{0.5}"  # Progress bars will show that IoU and it will be saved in checkpoints
    ...
    def run_epoch(self, split_name, dl, epoch, log_steps=None, opt=None, iter_step=None):
        ...
        epoch_iterator = dl
        if self.gpu == 0:
            epoch_iterator = tqdm(epoch_iterator, desc="{}: ".format(split_name), leave=False,position=0)
            # pycharm中多级进度条会导致多行显示，一般可以用勾选“emulate terminal in output console"来解决，
            # 但是pycharm远程interpreter无法启用此选项
            # 只是这样就看不到epoch的进度条了
        for i, batch in enumerate(epoch_iterator):
            ...
            # loss_extra_dict中存储有各个类别不同thre下的iou，以后可以分析是使用
    ...
    def fit(self, train_dl, val_dl=None, init_dl=None):
        ...
        if self.gpu == 0:
            fit_iterator = tqdm(fit_iterator, desc="Fitting: ", initial=start_epoch,leave=False,
                                total=self.config["optim_params"]["max_epoch"],position=0)
        ...
        for epoch in fit_iterator:
            ...
            print("epoch %d/%d finished,val iou: %f,val loss: %f"%(epoch,self.config["optim_params"]["max_epoch"],val_iou,val_loss))
    ...
    def load_checkpoint(self, checkpoints_dirpath):
        ...
        try:
            ...
            self.loss_func.load_state_dict(checkpoint['loss_func_state_dict'],strict=False)
