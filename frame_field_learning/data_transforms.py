...
class CudaDataAugmentation(object):
    def __init__(self, input_patch_size: int, vflip: bool, affine: bool, scaling: list, color_jitter: bool,rotate=True):
...
def off_nadir_dataseet_vis2class(tensor):
    ...
    for vis_value,class_num in channel_transforms_interior.items():
        tensor[:,0][torch.logical_and(tensor[:,0,...]<(pre_class_vis+vis_value)/2,tensor[:,0]>=pre_thre)]=class_num-1
        pre_thre=(pre_class_vis+vis_value)/2
        pre_class_vis=vis_value
    tensor[:,0][tensor[:,0,...]>=pre_thre]=class_num

    pre_class_vis =pre_thre =  background_vis
    for vis_value,class_num in channel_transforms_edge.items():
        tensor[:,1][torch.logical_and(tensor[:,1,...]<(pre_class_vis+vis_value)/2,tensor[:,1]>=pre_thre)]=class_num-1
        pre_thre = (pre_class_vis + vis_value) / 2
        pre_class_vis=vis_value
    tensor[:,1][tensor[:,1,...]>=pre_thre]=class_num

    pre_class_vis = pre_thre = background_vis
    for vis_value,class_num in channel_transforms_vertex.items():
        tensor[:,2][torch.logical_and(tensor[:,2,...]<(pre_class_vis+vis_value)/2,tensor[:,2]>=pre_thre)]=class_num-1
        pre_thre = (pre_class_vis + vis_value) / 2
        pre_class_vis=vis_value
    tensor[:,2][tensor[:,2,...]>=pre_thre]=class_num

    return tensor


def get_online_cuda_transform(config, augmentations=False):
    if "off_nadir" in config["dataset_params"] and config["dataset_params"]["off_nadir"]:
        device_transform_list = [
            ...
            torch_lydorn.torchvision.transforms.TransformByKey(
                transform=torchvision.transforms.Lambda(off_nadir_dataseet_vis2class),
                key="gt_polygons_image"),
            torch_lydorn.torchvision.transforms.TransformByKey(
    ...
    if config["data_aug_params"]["device"] == "cuda":
        ...
        if augmentations:
            rotate=config["data_aug_params"].get('rotate',True)
